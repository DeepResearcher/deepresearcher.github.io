(() => {
  ...
  function buildPortfolio() {
    ...
    let grid = section.querySelector(".portfolio-category-grid");
    if (!grid) {
      const previous = section.querySelector(".portfolio-slider, .portfolio-gallery, .concept-grid");
    ...
    grid.querySelectorAll(".portfolio-category").forEach((card, categoryIndex) => {
      ...
      dots.innerHTML = category.items.map((item, itemIndex) => `
        <button
          type="button"
          class="portfolio-category-dot${itemIndex === 0 ? " is-active" : ""}"
  ...
  function start() {
    ...
    if (!buildPortfolio()) {
      const observer = new MutationObserver(() => {
        if (buildPortfolio()) observer.disconnect();
      });
